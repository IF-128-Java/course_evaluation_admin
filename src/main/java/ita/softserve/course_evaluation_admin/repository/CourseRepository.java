...
public interface CourseRepository extends JpaRepository<Course, Long> {
    @Query(value = "SELECT c.id, c.course_name, c.description, c.start_date, c.end_date, " +
            ...
            "AND (CASE WHEN date_trunc('day',c.end_date) <= date_trunc('day',NOW()) THEN 'COMPLETED' WHEN date_trunc('day',c.start_date) > date_trunc('day',NOW()) THEN 'EXPECTED' WHEN date_trunc('day', c.start_date) <= date_trunc('day', NOW()) AND date_trunc('day', c.end_date) > date_trunc('day', NOW()) THEN 'ACTIVE' END) in (:status)"
    ...
    @Query(value = "SELECT DISTINCT c.id, c.course_name, c.description, c.start_date, c.end_date, c.teacher_id FROM course c " +
            ...
            "(SELECT course_id AS id FROM course_group WHERE group_id = ?1) AND date_trunc('day', c.end_date) > date_trunc('day', NOW()) AND date_trunc('day',c.start_date) >= (date_trunc('day',NOW()) - interval '7 day')"
